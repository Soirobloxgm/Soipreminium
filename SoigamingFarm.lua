...
Window = Fluent:CreateWindow({
    Title = "SóiGaming-Blox Fruit [ Preminium ] By Soihackroblox",
    SubTitle = "",
    TabWidth = 155,
...
Toggle = Settings:AddToggle("Toggle", {Title = "Gôm quái", Default = true})
Toggle:OnChanged(function(Value)
...
Main:AddButton({
    ...
    Callback = function()
        pcall(function()
            setclipboard("https://discord.gg/UnKY63cE")
        end)
    end
})
Main:AddParagraph({
...
Toggle = Main:AddToggle("Toggle", { Title = "Bắt đầu fram ", Default = false })
Toggle:OnChanged(function(Value)
    getgenv().AutoFarm = Value
    StopTween(getgenv().AutoFarm)
end)
Main:AddParagraph({
     Title = "Farming nguyên liệu",
     Content = string.rep("-", 21)
})
local MaterialList = {}
if World1 then
...
Toggle = Main:AddToggle("Toggle", {Title = "Farm nguyên liệu", Default = false})
Toggle:OnChanged(function(Value)
...
Toggle = Stack:AddToggle("Toggle", {Title = "Dịch chuyển đến trái [ Hop Server ]", Default = false })
Toggle:OnChanged(function(Value)
...
Other = Window:AddTab({ Title = "Tab Farming Khác", Icon = "" })
Other:AddParagraph({
        Title = "Quest Dragon",
        Content = ""
})
...
ToggleChest = Other:AddToggle("ToggleChest", {Title = "Auto Farm rương", Default = false })
...
ImageLabel.Image = "http://www.roblox.com/asset/?id= 116246887050298"
